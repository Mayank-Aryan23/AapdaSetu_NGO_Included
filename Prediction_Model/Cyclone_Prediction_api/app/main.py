...
from app.model import CycloneModel
...
app = FastAPI(
    ...
    description=(
        "Predicts cyclone risk from latitude & longitude.\n\n"
        "**Sources:**\n"
        "- Open-Meteo (sea surface temp, sea level pressure, CAPE)\n"
        "- OpenWeatherMap 2.5 (max wind speed)\n"
        "- Derived (cyclone season active flag)\n\n"
        "**Model defaults (overridable in POST body):**\n"
        "- ocean_heat_content_kjcm2 = 85.4\n"
        "- wind_shear_ms = 12.3\n"
        "- vorticity_850hpa = 0.00042\n"
        "- relative_humidity_500hpa = 62.1\n\n"
        "**Output:** `cyclone: 0 or 1` + prediction_score (2 dp) + confidence + verdict"
    ),
    version="1.0.0",
    contact={
        "name": "Cyclone AI",
...
async def root():
    return {
        "service":  "Cyclone Prediction API",
        "version":  "1.0.0",
        "status":   "running",
        "docs":     "/docs",
        "endpoints": {
...
@app.get(
    ...
    tags=["Prediction"],
    summary="Predict cyclone — GET",
)
async def predict_get(
    lat: float = Query(..., ge=-90,  le=90,  description="Latitude",  example=15.5),
    lon: float = Query(..., ge=-180, le=180, description="Longitude", example=82.3),
):
    """
    Returns cyclone prediction for the given coordinates.
...
async def get_features(
    ...
    """Returns the 9 features that would be fed into the model, without running inference."""
    req = PredictionRequest(lat=lat, lon=lon)
    try:
